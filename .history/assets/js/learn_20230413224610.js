...
document.body.addEventListener("click", function (e) {
    if (e.target.closest(".learn-content-item--listen")) {
        console.log(e.target.closest(".learn-content-item--listen"));
        e.target
            .closest(".learn-content-item--listen")
            .classList.toggle("learn-content-item--selected");

        const items = document.querySelectorAll(".learn-content-item");

        if (
            ![...items].filter((item) => {
                item.classList.contain("learn-content-item--selected");
            })
        )
            learnBtn.classList.remove("btn--primary");
        else learnBtn.classList.add("btn--primary");
...
function loadListen(data, len, thumb = true) {
    ...
    for (let i = 0; i < data.choices.length; i++) {
        ...
        const template = `<div class="learn-content-item learn-content-item--listen">
