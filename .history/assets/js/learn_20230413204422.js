async function getData(index) {
    ...
    console.log(
        data[index].adaptiveChallenges
            ? { ...data[index].adaptiveChallenges }
            : {}
    );
    const a = data[index].adaptiveChallenges? [...data[index].adaptiveChallenges] : {}
    return {data[index].adaptiveChallenges? [...data[index].adaptiveChallenges] : {}, [...data[index].challenges]};
}
...
async function loadLearn(index, thumb = true) {
    ...
    console.log();
    const len =
        (data.adaptiveChallenges ? data.adaptiveChallenges.length : 0) +
        data.challenges.length;

    switch (data.challenges[0].type) {
        case "select":
            loadSelect(data, len);
            break;
